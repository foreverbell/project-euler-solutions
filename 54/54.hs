...
consecutiveValue cs = vs == [head vs .. last vs]
    where vs = sort $ map cardValue cs

highCard :: [Card] -> Maybe [Int]
highCard cs = Just $ reverse vs
    where vs = sort $ map cardValue cs
...
calScore c = (10 - index, (\(Just x) -> x) (result!!index))
    where
